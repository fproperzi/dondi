...
*/	
	...
	$stmt = DB::run("INSERT OR IGNORE INTO t_users_access (user_id,dd,cnt) values (?,?,?)",[$user_id,date('Ymd'),1]);
	if($stmt->rowCount() == 0 ) {
		echo "update<br>";
		$stmt = DB::run("UPDATE t_users_access SET cnt=cnt+1 WHERE user_id=? AND dd=?",[$user_id,date('Ymd')]);
	}
