...
class NestedSet extends DB {
	...
	public function checkPath($path) {
		if (substr ( $path, 0, strlen(self::C_ROOT_LABEL) ) !== self::C_ROOT_LABEL) 
			$path = self::C_ROOT_LABEL . ($path[0] !== "/" ? "/" : "") . $path; 
	...
	public function fixPath($path, $suffix=="") { 
		if ($path[0] === "/")                   $path = substr ( $path, 1);  						// role/editor/
		if ($path [strlen( $path ) - 1] == "/")	$path = substr ( $path, 0, strlen ( $path ) - 1 );	// role/editor
		if (substr ( $path, 0, strlen(self::C_ROOT_LABEL) ) == self::C_ROOT_LABEL) 
			                                    $path = substr ( $path, strlen(self::C_ROOT_LABEL)+1 );
		
		if(strlen($suffix)!=0)					$path = self::C_ROOT_LABEL ."/". $suffix ."/". $path;
		else 				                    $path = self::C_ROOT_LABEL ."/". $path;  
    ...
    // add all nodes in path if not exists, return leaf: root/roles/staff/tecnical/coach -> return coach role_id 
...
class Rbac extends NestedSet { 
	...
	private function getNode($ppath, &$s) { //any &$s change is not lost
		if (is_numeric($s)) 
			return $s;
		//$s = $ppath . ($s[0] !== "/" ? "/$s" : $s);
		$s = $this->fixPath($s,$ppath);
		return  $this->path2Id( $s );
	}
	private function setNode($ppath, $s) {	
		$id = $this->getNode($ppath, $s);
		if($id) return $id; 
		else    return $this->addPath( $s ); 
	}
	
	public function setRoleId($role) { return $this->setNode(self::C_ROLES,$role); }
	public function getRoleId($role) { return $this->getNode(self::C_ROLES,$role); }	
	public function setPermId($perm) { return $this->setNode(self::C_PERMS,$perm); }
...
*/
	...
	*/
	...
	$b->setRole2Perm("staff/tecnical/coach"		,"page/medical/comments"); 
	$b->setRole2Perm("staff/tecnical/referee"	,"page/medical/comments/read");
	$b->setRole2Perm("staff/medical/doctor"		,"page/medical/comments/edit");
	
	
	$b->setUser2Role(65				,"staff/tecnical/coach");
	$b->setUser2Role(65				,"editor"); 
	$b->setUser2Role(66				,"staff"); 
	
	$user_id = 65;
	$perm = "page/clip/commentts";
