...
**/
define ("C_CONFIG_DB_HOST"      ,"127.0.0.1");
define("C_CONFIG_DB_USER"       ,"u_mclips" );
define("C_CONFIG_DB_PASS"       ,"nnkk219"  );
define("C_CONFIG_DB_NAME"       ,"mclips"   );

require_once ("rr_config_func.01.php");
require_once ("rr_crud.01.php"); 
...
$ga_sets = array(             /** _L+_O+_2I+_2U+_2D+_2C+_I+_U+_D+_C ** required field for action **/                                                                                                        
    ...
    ),'lkp_flag1'   => array("a"=>_N                                 ,"l"=>"linked to"  ,"t"=>'select'  ,"p"=>"...","filter"=>FILTER_CALLBACK        ,"flags"=>FILTER_REQUIRE_SCALAR ,"options"=>function($v) { return bfInArray($v,aofLkSets() );}  ,"o"=>aofLkSets()   
    )                                                                                                                                              
);    
/* used in linked to
**/
function aofLkSets($i=0) {
	$s = "select lkp_id,lkp_name from t_lookup where lkp_set=$i order by lkp_name"; global $dbg; $dbg['LkSets']=$s;


	$rs = mysql_query($s); $avar = array();  
	if($rs)while ($r = mysql_fetch_assoc($rs)) 
		$avar[ $r['lkp_id'] ] = $r['lkp_name']; 
	return $avar;
}                                                                                                                                             
...
$ga_tags = array(             /** _L+_O+_2I+_2U+_2D+_2C+_I+_U+_D+_C ** required field for action **/                                               
    ...
    ),'lkp_note'    => array("a"=>_N                                 ,"l"=>"Note"       ,"t"=>'text'    ,"p"=>"","filter"=>FILTER_VALIDATE_REGEXP ,"flags"=>FILTER_REQUIRE_SCALAR ,"options"=>array("regexp"=>C_REGEXP_LKP_TEXT) ,"h"=>C_HELP_LKP_TEXT                                                                                                                                               
    ),'lkp_order'   => array("a"=>   _O                              ,"l"=>"Order"      ,"t"=>'hidden'  ,"p"=>"","filter"=>FILTER_VALIDATE_INT    ,"flags"=>FILTER_REQUIRE_SCALAR                                                                                     
    ),'lkp_text'    => array("a"=>_N                                 ,"l"=>"linked to"  ,"t"=>'select'  ,"p"=>"...","filter"=>FILTER_CALLBACK     ,"flags"=>FILTER_REQUIRE_SCALAR ,"options"=>function($v) { return true;}  ,"o"=>array()   
	)                                                                                                                                                                                                        
);                                                                                                                                                                                                           
...
if(!empty($_REQUEST['action'])) {
    ...
    try { 
        ...
        if(empty($lkp_set))  {
			...
			$s = "select s.lkp_flag1 from t_lookup s inner join t_lookup t on s.lkp_id=t.lkp_set  where t.lkp_id=$lkp_id";    $dbg['flag1_sql']=$s;
			$rs = mysql_query($s);  
			if($rs) { 
				$r = mysql_fetch_assoc($rs);
				if ($r['lkp_flag1'] == 0) unset($avar['lkp_text']);
				else $avar['lkp_text']['o'] = aofLkSets($r['lkp_flag1']);
				
				 $dbg['flag1']=$r['lkp_flag1'];
...
<html><head>
<?= sfHead("config"
            ,array("js/rr_crud.js","//cdnjs.cloudflare.com/ajax/libs/slipjs/2.0.0/slip.min.js")
...
function vfActionSwitch(a,r) {
    ...
    /****/ case "set_list": 
        ...
        $('#page-tag-sets .p-action' ).off('click')  // return false if not select view tag_list  href="#"
                                       .on('click',function(){ vfActionLoad( $(this).data(),vfActionSwitch );
													if(empty($(this).attr("href")) || $(this).attr("href").length==1) return false;});           
...
$(document).on("pagecreate", "#page-tag-tags", function() {
	vfSlipInit("#page-tag-tags-ul","lkp_order",vfActionSwitch)
});
    ...
    <%  _.each(l,function(i){   %>
        <li data-action="set_order" data-lkp_id="<%= i.lkp_id %>" data-lkp_order="<%= i.lkp_order %>">
			<a href="#page-tag-tags" data-action="tag_list" data-lkp_set="<%= i.lkp_id %>" data-lkp_set_name="<%= i.lkp_name %>" class="p-action">
                <%= i.lkp_name %>    
                <div class="ui-btn-right ui-nodisc-icon" style="top:50%;margin-top:-15px">
					<button data-action="set_2update"  data-lkp_id="<%= i.lkp_id %>"  class="p-action ui-btn ui-shadow ui-corner-all ui-icon-edit ui-btn-icon-notext ui-btn-inline"></button>                
    ...
    <%  _.each(l,function(i){   %>
        <li data-action="tag_order" data-lkp_id="<%= i.lkp_id %>"  data-lkp_set="<%= i.lkp_set %>" data-lkp_order="<%= i.lkp_order %>" >
            <a href="#" data-action="tag_2update" data-lkp_id="<%= i.lkp_id %>" class="p-action">
                <%= i.lkp_name %> 
				<% if(!empty(i.lkp_note)){ %><p><%= i.lkp_note %></p><% }; %>
                <div class="ui-btn-right ui-nodisc-icon" style="top:50%;margin-top:-15px">
    ...
    <body>
        ...
        <div id="page-tag-sets" data-role="page" data-theme="b">
            ...
            <div data-role="content" >  
                <ul id="page-tag-sets-ul" data-role="listview" data-filter="false" data-split-icon="bars" ></ul>
            </div> <!-- /content -->    
        </div> <!-- /page-sets -->
        
        <div id="page-tag-tags" data-role="page" data-theme="b">
            <div data-role="header" data-position="fixed"  data-tap-toggle="false" >
                <a href="#page-tag-sets" data-icon="back" data-iconpos="notext">Back</a>
                <h1>Items</h1>
