...
<!DOCTYPE html> 
<html><head>
<?= sfHead("config",array("js/rr_crud.js","js/jquery-ui-sortable.min.js"),array("css/rr_crud.css")) ?>
...
$(document).on("pageshow", "#page-tag-tags", function() {
    //$('#page-tag-tags-ul').empty(); // avoid view last list in memory till reload (for few seconds)
	        $('#page-tag-tags-ul').sortable({ //http://jsfiddle.net/sidonaldson/pes2d/
            'containment': 'parent',      // https://api.jqueryui.com/sortable/#event-update
            'opacity': 0.6,
            update: function(event, ui) { 
				$(ui.item[0]).jqmData('lkp_order',ui.item[0].childElementCount-1);
				vfActionLoad( _.omit($(ui.item[0]).data(),'sortableItem'),vfActionSwitch );
            }
        });
});
...
<script type="text/template" id="page-tag-sets-ul-tmpl">
    <%  _.each(l,function(i){   %>
        <li data-action="set_order" data-lkp_id="<%= i.lkp_id %>" data-lkp_order="<%= i.lkp_order %>">
            ...
            <a href="#" data-action="set_2delete" data-lkp_id="<%= i.lkp_id %>"  class="p-action">Delete</a> 
        </li>
    <% }); %>
</script>
<script type="text/template" id="page-tag-tags-ul-tmpl">
    <%  _.each(l,function(i){   %>
        <li data-action="tag_order" data-lkp_id="<%= i.lkp_id %>"  data-lkp_set="<%= i.lkp_set %>" data-lkp_order="<%= i.lkp_order %>" >
            ...
            <a href="#" data-action="tag_2delete" data-lkp_id="<%= i.lkp_id %>"  data-lkp_set="<%= i.lkp_set %>"  class="p-action">Delete</a> 
        </li>
    <% }); %>
</script>
