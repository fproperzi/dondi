...
$ga_sets = array(             /** _L+_O+_2I+_2U+_2D+_2C+_I+_U+_D+_C ** required field for action **/                                                                                                        
    ...
    ),'lkp_name'    => array("a"=>                      _I+_U   +_C  ,"l"=>"Set lookup" ,"t"=>'text'    ,"p"=>"" ,"filter"=>FILTER_VALIDATE_REGEXP ,"flags"=>FILTER_REQUIRE_SCALAR ,"options"=>array("regexp"=>"/^[[:graph:][:space:]]{1,255}$/") ,"h"=>""                             
        ...
        break; case 'tag_2insert': $action = "tag_insert";
            ...
            foreach($avar as $n => $d) { //load values from existing record to update                   
                ...
                break; case "lkp_order"  : // prepare with order last record
                    $s = "select max(lkp_order)+1 from t_lookup where lkp_set='{$avar['lkp_set']['v']}'"; // max order for set then in insert adjust
                    $avar[$n]['v'] = sfSQL2Cnt( $s ); $dbg['sql'] = $s;
