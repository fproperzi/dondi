...
class DB {
    ...
    public static function instance() {
        if (self::$instance === null) {
            $opt  = array(
                PDO::ATTR_ERRMODE            => PDO::ERRMODE_EXCEPTION, //try { DB::run()->...() } catch(PDOException $e){}
                PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
                PDO::ATTR_EMULATE_PREPARES   => FALSE,
            );
            //$dsn = 'mysql:host='.DB_HOST.';dbname='.DB_NAME.';charset='.DB_CHAR;
            //self::$instance = new PDO($dsn, DB_USER, DB_PASS, $opt);
            self::$instance = new PDO("sqlite:".dirname(__FILE__)."/db/ra.sqlite3","","",$opt);
...
try {
$r = DB::run("SELECT *,rowid FROM t_clip_source1")->fetchAll();
} catch(Exception $e){
    echo $e->getMessage();
}
