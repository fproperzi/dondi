...
function vfActionSwitch(a,r) {
	...
	/****/ case "set_list": 
		...
		else vfPopUp('Error','<p class="error">'+ r.message + '</p>'); 
  	...
  	break; case "set_2insert":	case "tag_2insert":	if(empty(c)){c = "insert"; h = "New"   ; }
           ...
           case "set_2copy"  :  case "tag_2copy"  : if(empty(c)){c = "copy"  ; h = "Copy"  ; s  = "<h3>Do you want to copy this %s?</h3>";}
           case "set_2delete":  case "tag_2delete": if(empty(c)){c = "delete"; h = "Delete"; s  = "<h3>Do you want to delete this %s?</h3><p>This action can not be undone.</p>"; }

		if(r.status) {
			s = sprintf(s,(a.substr(0,3) === "tag" ? "TAG" : "SET and all his TAGS");
			s +=  sfBuidForm(r,c);
	...
	break; case "tag_list":
		...
		if(r.status) {
            ...
            $('#page-tag-tags .p-action-new').jqmData("set_id", set_id);
